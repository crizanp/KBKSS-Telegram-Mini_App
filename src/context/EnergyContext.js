...
useEffect(() => {
  if (isCooldownActive) {
    ...
    if (now >= cooldownEnd) {
      // Cooldown is over, reset energy to 1 and stop regeneration during cooldown
      setEnergy(1); // Start regeneration from 1
      ...
      setIsCooldownActive(false); // Cooldown ends, resume normal energy regeneration
      localStorage.removeItem(`cooldownEnd_${USER_ID}`); // Remove cooldown marker from localStorage
      return;
    }
    ...
    const cooldownInterval = setInterval(() => {
      ...
      if (updatedCooldownTime <= 0) {
        ...
        setEnergy(1);  // Cooldown ends, reset energy to 1
        localStorage.setItem(`energy_${USER_ID}`, '1');  // Update local storage to 1
        setIsCooldownActive(false);  // End cooldown
        setCooldownTimeLeft(0);  // Reset cooldown time
        localStorage.removeItem(`cooldownEnd_${USER_ID}`);  // Clean up cooldown from localStorage
      } else {
...
useEffect(() => {
  if (!isEnergyReady || isCooldownActive) return; // Do not regenerate during cooldown
